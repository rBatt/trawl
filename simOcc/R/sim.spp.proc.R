#' Simulate Species Process
#' 
#' Simulate species occupying an environmental grid
#' 
#' @param grid.X environmental grid  
#' @param ns number of species  
#' @param alpha_mu numeric vector of length 3, given the means of the intercept X coefficient, and X^2 coeff. X is the environmental varaible in grid.X.  
#' @param alpha_sd the standard deviations associated with each of the terms described for \code{alpha_mu}  
#' @param dynamic default TRUE means that species can only move to adjacent cells between years. FALSE means species aren't spatially restricted, and each year is indepenedent from the previous in terms of how many locations a species is occupying (independent given environmental grid)  
#' @param AR1.coef If dynamic is TRUE, this is the autocorrelation coefficient of site and species-specific biomass between time steps  
#' @param D.frac Dispersal fraction; if dynamic is TRUE, fraction of biomass to disperse from local cell to surroundings; think of it like reproduction, such that it doesn't result in a decrease in the biomass of the focal cell  
#' @param M.frac Movement fraction; if dynamic is TRUE, proportion of biomass that will leave cell when it becomes unsuitable (this proportion is split evenly among neighbors, but not all neighbors will be suitable, so in many cases less than 100 percent of the biomass will find a new home)  
#' @param perist.bonus Peristence bonus; if dynamic is TRUE, factor by which to adjust the probability that a species will fail to persist; decreasing this factors makes the species more likely to stick around, increasing it makes it less likely to stick around. When 1, no adjustment is made; when 0, the species will always stick around.  
#' 
#' @details
...
#' @import raster
#' @export
sim.spp.proc <- function(grid.X, ns=200, alpha_mu=c(0.5, 0.00, -0.025), alpha_sd=c(0.5, 0.21, 0.008), dynamic=FALSE, AR1.coef=0.8, D.frac=0.5, M.frac=1, persist.bonus=1){
	
	stopifnot(length(alpha_mu)==3)
	stopifnot(length(alpha_sd)==3)
	...
	u.a0 <- rnorm(ns, alpha_mu[1], alpha_sd[1])
	a3 <- rnorm(ns, alpha_mu[2], alpha_sd[2]) #~ dnorm(0, 0.001)
	a4 <- rnorm(ns, alpha_mu[3], alpha_sd[3]) #~ dnorm(0, 0.001)
	...
	p.suit2 <- simplify2array(mapply(function(...)psiMod(...)$y, u.a0, a3, a4, MoreArgs=list(X=values(raster::subset(grid.X, 1))), SIMPLIFY=F)) # this can now go to colonize() instead of having to rely on dsample(); I tested, and it shouldn't matter much provided that a large enough reference sample size is used.
...
#
	...
	c0 <- colonize(values(raster::subset(grid.X, 1)), S.dens.X, spp.bio.mu, relative=FALSE, probs=p.suit2)
	...
	adjac <- r2m.cell(adjacent(raster::subset(grid.X, 1), cells=1:Nv, direction=8), nrow=grid.h, ncol=grid.w)
