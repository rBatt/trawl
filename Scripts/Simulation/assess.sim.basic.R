...
#' In these "time series" plots (so short!), the replciates (grey lines) look so jagged and incosistent because each replicate shifts which year gets which value of $p_{\mu}$ (average detectability). It's interesing to note here that the bottom panel tells us that the MSOM expects our data to have lower richness than it really does.  
...
#' \end{array} \right) $$  
#'   
#' Therefore, these curves are tantamount to values of $\psi$, except that $\psi$ generally pertains to a simulated, observed, or true occupancy probability, whereas the occupancy probability in the response curves is calculated over hypothetical conditions (i.e., over hypothetical values of the environmental gradient $X$).  This formulation yields value of $\psi$ for each species at all temperatures over the interval $(X_{min}, X_{max})$ (this implies that the number of rows of $X$ is infinite, which it isn't, but it is quite large).  
